...
# Tröskel för semantisk relevans (bestämd via mathtest.py på ett separat test-dataset).
# Värdet 0.57 ger optimal balans (maximerad F1-score) med 100% precision 
# för att skilja relevanta ämnesmatchningar från irrelevanta.
            ...
            try:
                ...
                if party_name:
                    ...
                    for pillar, items in pillars_dict.items():
                        ...
                        # --- HJÄLPFUNKTION FÖR ATT SEPARERA SBERT OCH NLI ---
                            ...
                            if isinstance(item_data, str) and item_data.strip():
                                ...
                                if stance.strip():
                                    ...
                                    return {
                                        ...
                                        "nli_text": stance.strip(), # Enbart ståndpunkten för NLI
                                        "original": sbert_text
                                    }
                            return None
    ...
    for pillar, user_chunks in active_routing.items():
        ...
        for party, party_statements in parties.items():
            ...
            if not party_statements or not user_chunks:
                pillar_party_scores[pillar][party] = 0.0
                continue
                
            # --- ANVÄND SBERT-TEXTEN FÖR MATCHNING ---
            ...
            for u_idx, user_chunk in enumerate(user_chunks):
                ...
                sbert_scaled = max(0.0, min(1.0, (sbert_peak - MIN_BOUND) / (MAX_BOUND - MIN_BOUND)))
                
                # --- STRIP LEADING AFFIRMATIVES FÖR NLI ---
                # NLI-modellen får enbart 'nli_text' (själva svaret/ståndpunkten) utan "Ja/Nej"
                ...
                chunk_score = max(0.0, min(1.0, (sbert_scaled * contradiction_multiplier) + entailment_boost))
                ...
                inference_traces[pillar][party].append({
                    "user_chunk": user_chunk,
                    "party_statement": best_match["original"], # Vi visar fortfarande hela prompten+svaret i UI:t
                    "score": chunk_score * 100,
                    "nli_status": nli_status,
                    "entailment": avg_entail * 100,
                    "contradiction": avg_contra * 100
                })
            
            pillar_party_scores[pillar][party] = (sum(chunk_scores) / len(chunk_scores)) if chunk_scores else 0.0
